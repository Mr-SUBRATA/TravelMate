...
class GroupHarmonyOptimizer:
    ...
    def _build_group_itinerary(self,
        ...
        for day in range(days):
            ...
            if chosen_activity:
                # Calculate happiness for all users for this primary activity
                day_scores = {}
                for user in users:
                ...
                # Store a copy of the chosen activity annotated with scores,
                # so fairness metrics can read them later.
                annotated_activity = dict(chosen_activity)
                annotated_activity['scores'] = day_scores
                annotated_activity['priority_user'] = priority_user['name']
                day_activities.append(annotated_activity)
