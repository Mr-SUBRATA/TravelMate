...
class TravelRecommender:
    ...
    async def recommend_from_live_data(self,
        ...
        for place in places:  # ← Loop through whatever backend sent
            ...
            place_vector = self._categories_to_vector(categories)
            
            # Calculate similarity with user (cosine)
            similarity = cosine_similarity(
                user_vector.reshape(1, -1),
            ...
            weather_score = 0.8
            is_outdoor = any(
                key in " ".join(categories).lower()
                for key in ["outdoor", "mountain", "mountains", "trekking"]
            )
            if weather_data:
            ...
            # Estimate cost from price_level (1-4 scale)
    ...
    def _categories_to_vector(self, categories: List[str]) -> np.ndarray:
        ...
        # Category to dimension mapping (expanded for broader places)
        category_map = {
            ...
            'adventure_sport': (48, 50), 'hiking': (48, 50), 'climbing': (48, 50),
