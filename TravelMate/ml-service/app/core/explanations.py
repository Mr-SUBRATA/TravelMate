...
class ExplanationGenerator:
    ...
    def _explain_preferences(self, recommendation: Dict, preferences: Dict) -> List[str]:
        """Explain based on user's stated preferences (quiz_answers)."""
        reasons: List[str] = []

        # Normalize quiz field names (art_interest, foodie_score, etc.) to internal keys
        normalized_prefs: Dict[str, int] = {}
        if "art_interest" in preferences:
            normalized_prefs["art"] = preferences["art_interest"]
        if "foodie_score" in preferences:
            normalized_prefs["food"] = preferences["foodie_score"]
        if "adventure_seeking" in preferences:
            normalized_prefs["adventure"] = preferences["adventure_seeking"]
        # These quiz fields don't directly map to a single category, but we can still
        # use them later if you add more nuanced explanations.

        # Map recommendation categories to preference types
        category_map = {
            "art": ["museum", "gallery", "art"],
            "food": ["restaurant", "food", "cafe", "bakery", "street_food", "biryani"],
            "adventure": ["hiking", "adventure", "extreme", "outdoor", "trekking"],
            "history": ["historical", "monument", "heritage", "ancient", "fort"],
            "shopping": ["mall", "shop", "boutique", "market", "bazaar"],
            "nature": ["park", "garden", "nature", "beach", "valley", "mountain"],
        ...
        # Strong preference (4–5) that matches categories
        for pref_key, score in normalized_prefs.items():
            if score >= 4 and pref_key in category_map:
                if any(cat in rec_categories for cat in category_map[pref_key]):
                    template = self.reason_templates["preference"].get(pref_key)
                    if template:
                        reasons.append(template.format(score))

        # If no specific matches, fall back to top interest
        if not reasons and normalized_prefs and rec_categories:
            top_pref_key, top_score = max(normalized_prefs.items(), key=lambda x: x[1])
            if top_score >= 3:
                reasons.append(f"Matches your interest in {top_pref_key}")
